...
/// 请求行情信息
func (m *Market) RequestForTime(req string, from string) (*simplejson.Json, error) {
	var id = getRandomString(10)
	m.requestResultCb[id] = make(jsonChan)

	if err := m.sendMessage(reqDataTime{Req: req, ID: id, From: from}); err != nil {
		return nil, err
	}
	var json = <-m.requestResultCb[id]
